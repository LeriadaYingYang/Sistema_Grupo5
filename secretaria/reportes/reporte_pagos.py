...
def reporte_por_metodo_pago(): # Filtra pagos según el método de pago ingresado
    ...
    encontrados = 0
    for pago in pagos:
        if (pago["metodo_pago"].lower() == metodo):
            mostrar_pago(pago)
            encontrados += 1
    if encontrados == 0:
...
# Conversión segura a entero
    try:
        id_pago = int(input("Ingresar ID pago: "))
    except ValueError:
        print("Debe ingresar un número.")
        return
    pago = buscar_pago_por_id(pagos,id_pago)
    if pago is None:
        print("Pago no encontrado.")
        return
    imprimir_titulo("=== DATOS PAGO ===")
...
""")
        ...
        match opcion:
            case "1":reporte_general_pagos()
            case "2":reporte_pagos_pagados()
            case "3":reporte_pagos_pendientes()
            case "4":reporte_por_metodo_pago()
            case "5":reporte_mayores_montos()
            case "6":buscar_pago()
            case "7":estadisticas_pagos()
            case "8":reporte_ordenado_alumnos()
            case "9":
