...
def calcular_deuda_alumno(alumno, plantilla, salon, cargos_oficiales, descuentos, cargos_extras, pagos):
    ...
    for cargo in cargos_oficiales:
        descuento = obtener_descuento_alumno(
            descuentos,
            alumno["id_alumno"],
            cargo["id_cargo_oficial"])
...
def mostrar_resumen_alumno(resumen):
    ...
    for item in resumen["detalle_oficial"]:
        print(
            f"- {item['cargo']} | "
            f"Original: S/ {item['monto_original']} | "
            f"Descuento: {item['descuento']} | "
            f"Final: S/ {item['monto_final']}")
...
def resumen_por_salon():
    ...
    cargos_oficiales = obtener_cargos_oficiales(
        cargos,
        plantilla["id_plantilla"],
        plantilla["id_carrera"])
