...
def eliminar_cargo_extra_logico():
    imprimir_titulo("5.3.4. ELIMINAR CARGO EXTRA")
    cargos_extras = leer_json(RUTA_CARGOS_EXTRAS)
    
    cargos_activos = [c for c in cargos_extras if c["estado"] == "Activo"]
    
    if len(cargos_activos) == 0:
        print("No se registran cargos extras activos en el sistema.")
        return

    print("\nLista de Cargos Extras Activos:")
    for ce in cargos_activos:
        nombre_destino = ce.get("nombre_alumno", "Toda la Carrera/Salón")
        print(f"ID: {ce['id_cargo_extra']} | {ce['nombre']} - S/ {ce['monto']} | Asignado a: {nombre_destino}")
        
    try:
        id_eliminar = int(input("\nIngrese el ID del cargo extra que desea eliminar: "))
        target = None
        for ce in cargos_extras:
            if ce["id_cargo_extra"] == id_eliminar and ce["estado"] == "Activo":
                target = ce
                break
                
        if target:
            target["estado"] = "Inactivo"
            guardar_json(RUTA_CARGOS_EXTRAS, cargos_extras)
            print(f"\n El cargo extra '{target['nombre']}' ha sido eliminado lógicamente con éxito.")
        else:
            print("\n ID no válido o el cargo ya se encuentra inactivo.")
    except ValueError:
        print("\n Error de entrada: Ingrese un valor numérico.")
