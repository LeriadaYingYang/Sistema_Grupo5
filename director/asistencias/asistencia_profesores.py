...
def obtener_asignaciones_profesor(asignaciones, id_profesor):
    resultado = []
...
def mostrar_plantillas_del_profesor(plantillas, asignaciones_profesor):
...
def mostrar_salones_profesor_por_plantilla(asignaciones_profesor, plantilla):
    ...
    for asignacion in asignaciones_profesor:
        if asignacion["id_carrera"] == plantilla["id_carrera"]:
            encontrados += 1
            print(
                f"ID salón: {asignacion['id_salon']} | "
...
def mostrar_modulos(modulos, id_plantilla, id_salon):
    print("\n=== MÓDULOS DISPONIBLES ===")
...
def asistencia_profesor_ya_registrada(asistencias, id_profesor, fecha, id_horario, orden_dia):
...
def registrar_asistencia_profesores():
    print("\n====================================")
    print("   REGISTRAR ASISTENCIA PROFESORES")
    print("====================================")
    ...
    for asignacion in asignaciones_profesor:
        if (
            ...
            and asignacion["id_carrera"] == plantilla["id_carrera"]):
    ...
    if (
        modulo is None
        or modulo.get("id_plantilla") != id_plantilla
        or modulo.get("id_salon") != id_salon):
    ...
    if asistencia_profesor_ya_registrada(
        asistencias,
