...
def buscar_alumno_aproximado_en_asistencias(asistencias):
    ...
    for alumno in encontrados:
        print(f"ID: {alumno['id_alumno']} | {alumno['nombre_alumno']} | DNI: {alumno['dni']}")
...
def filtrar_por_plantilla_y_salon(asistencias):
...
def imprimir_asistencia(asistencia):
    print("\n-----------------------------")
...
def ver_por_fecha():
    ...
    asistencias_fecha = [
        asistencia for asistencia in asistencias
        if asistencia["estado"] == "Activo" and asistencia["fecha"] == fecha]
    ...
    if opcion == "si":
        ...
        asistencias_fecha = [
            asistencia for asistencia in asistencias_fecha
...
def ver_resumen_porcentaje():
    ...
    if opcion == "si":
        ...
        asistencias_filtradas = [
            asistencia for asistencia in asistencias_filtradas
            if asistencia["id_alumno"] == alumno["id_alumno"]]
    ...
    for datos in resumen.values():
        porcentaje = round(
            ((datos["presentes"] + datos["tardes"] + datos["justificados"]) / datos["total"]) * 100,
            2)
...
def menu_ver_asistencia_alumnos():
    while True:
        print("""
====================================
