...
def consultar_horarios():
    ...
    opcion = input("\nSeleccione opción: ")
    if opcion == "1":
        mostrar_horarios([h for h in horarios if h["estado"] == "Activo"])
    elif opcion == "2":
        nombre = input("Nombre de plantilla: ").strip().lower()
        filtrados = [h for h in horarios if h["estado"] == "Activo" and nombre in h["nombre_plantilla"].lower()]
        mostrar_horarios(filtrados)
    elif opcion == "3":
        nombre = input("Nombre de carrera: ").strip().lower()
        filtrados = [h for h in horarios if h["estado"] == "Activo" and nombre in h["nombre_carrera"].lower()]
        mostrar_horarios(filtrados)
    elif opcion == "4":
        nombre = input("Nombre de salón: ").strip().lower()
        filtrados = [h for h in horarios if h["estado"] == "Activo" and nombre in h["nombre_salon"].lower()]
        mostrar_horarios(filtrados)
    else:
