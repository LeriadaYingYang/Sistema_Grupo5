...
def ver_carga_horaria_docente():
    ...
    if profesor is None:
        ...
        return
    asignaciones_profesor = [a for a in asignaciones if a["estado"] == "Activo" and a["id_profesor"] == id_profesor]
    ...
    for asignacion in asignaciones_profesor:
        horario = buscar_por_id(horarios,"id_horario",asignacion["id_horario"])
        if horario is None:
            continue
        print(f"\nPlantilla: {horario['nombre_plantilla']}")
        print(f"Carrera: {horario['nombre_carrera']}")
        print(f"Salón: {horario['nombre_salon']}")
        print(f"Turno: {horario['turno']}")
        print("\nDÍAS ASIGNADOS:")
        for dia in horario["dias_horas"]:
            horas_dia = calcular_horas(dia["hora_inicio"], dia["hora_fin"])
