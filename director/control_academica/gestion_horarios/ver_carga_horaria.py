...
def mostrar_profesores(profesores):
    ...
    for profesor in profesores:
        if profesor.get("estado") == "Activo":
            encontrados += 1
            print(f"ID: {profesor.get('id_profesor','N/A')} | "
...
def ver_carga_horaria_docente():
    ...
    profesores_activos = [p for p in profesores if p.get("estado") == "Activo"]
    if not profesores_activos:
        print("No existen profesores activos.")
        return
    asignaciones_activas = [a for a in asignaciones if a.get("estado") == "Activo"]
    ...
    asignaciones_profesor = [
        ...
        if a.get("id_profesor") == id_profesor]
    ...
    for asignacion in asignaciones_profesor:
        horario = buscar_por_id(horarios,"id_horario",asignacion.get("id_horario"))
        if horario is None:
            continue
        total_horarios += 1
        print(f"\nPlantilla: "
        ...
        for dia in dias_horas:
            ...
            hora_fin = dia.get("hora_fin","")
            horas_dia = calcular_horas(hora_inicio,hora_fin)
            total_horas += horas_dia
            print(f"{dia.get('dia','N/A')} | "
                f"{hora_inicio} - "
                f"{hora_fin} "
                f"({horas_dia} horas)")
        print("-" * 40)
