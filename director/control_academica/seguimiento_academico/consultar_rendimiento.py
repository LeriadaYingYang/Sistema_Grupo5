...
def consultar_rendimiento_academico():
    ...
    if not alumnos:
        print("No existen alumnos con registros activos.")
        return
    print("\nALUMNOS DISPONIBLES")
    for (id_alumno,nombre) in sorted(alumnos.items()):
    ...
    registros = [
        ...
        if (isinstance(nota,dict)
            ...
            and nota.get("id_alumno") == id_alumno)]
    if not registros:
        print(
            "No existen notas para este alumno.")
        return
    imprimir_titulo("RENDIMIENTO ACADÉMICO")
    nombre_alumno = registros[0].get("nombre_alumno","Sin nombre")
    print(f"\nAlumno: "
        f"{nombre_alumno}")
    suma = 0
    cantidad = 0
