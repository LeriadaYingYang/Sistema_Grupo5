...
def reporte_academico_general():
    ...
    for nota in notas:
        ...
        promedio_modulo = convertir_float(nota.get("promedio_modulo"))
        if id_alumno is None or promedio_modulo is None:
            continue
        registros_validos += 1
        if id_alumno not in alumnos:
            alumnos[id_alumno] = {"nombre": nombre_alumno,"suma": 0,"cantidad": 0}
    ...
    for datos in alumnos.values():
        ...
        if cantidad <= 0:
            continue
        promedio = round(datos["suma"] / cantidad,2)
        total_alumnos += 1
        ...
        if promedio > mejor_promedio:
            ...
            mejor_alumno = datos.get("nombre","Sin nombre")
        if promedio < peor_promedio:
            peor_promedio = promedio
            peor_alumno = datos.get("nombre","Sin nombre")
    if total_alumnos == 0:
    ...
    print(f"Total de alumnos evaluados: "
        f"{total_alumnos}")
    print(f"Promedio académico general: "
        f"{promedio_general}")
    print(f"Aprobados (>=13): "
        f"{aprobados}")
    print(f"Desaprobados (<13): "
        f"{desaprobados}")
    print(f"\nMejor alumno: "
        f"{mejor_alumno}")
    print(f"Promedio: "
