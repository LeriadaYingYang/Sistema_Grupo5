...
def ver_horas_trabajadas():
    ...
    for asistencia in asistencias:
        if asistencia.get("estado") == "Activo":
            profesores[asistencia["id_profesor"]] = asistencia.get("nombre_profesor",f"Profesor {asistencia['id_profesor']}")
    if not profesores:
    ...
    registros = [asistencia for asistencia in asistencias
        if (asistencia.get("estado") == "Activo" and asistencia["id_profesor"] == id_profesor)]
    if len(registros) == 0:
