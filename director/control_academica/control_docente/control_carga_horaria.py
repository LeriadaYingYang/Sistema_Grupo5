...
def convertir_hora(hora):
    try:
        ...
        if "a" in hora:
            ...
            if inicio.isdigit():
                return datetime.strptime(f"{int(inicio):02d}:00","%H:%M")
        if hora.isdigit():
            return datetime.strptime(f"{int(hora):02d}:00","%H:%M")
    except:
        pass
    return None
...
def control_carga_horaria():
    ...
    try:
        ...
        return
    profesor = next((p for p in profesores if p["id_profesor"] == id_profesor and p["estado"] == "Activo"),None)
    if profesor is None:
    ...
    for asistencia in asistencias:
        if (asistencia.get("id_profesor") == id_profesor):
            horas_trabajadas += asistencia.get("horas_trabajadas",0)
    horas_programadas = calcular_horas_programadas(horarios)
    if horas_programadas <= 0:
        cumplimiento = 0
    else:
        cumplimiento = round((horas_trabajadas/ horas_programadas)* 100,2)
