...
def mostrar_carreras_desde_salones(salones):
...
def mostrar_plantillas_por_carrera(plantillas, id_carrera):
...
def salon_ya_tiene_unidades(unidades, id_salon, id_plantilla):
...
def mostrar_salones_disponibles(salones, unidades, id_carrera, id_plantilla):
    ...
    for salon in salones:
        if salon["estado"] == "Activo" and salon["id_carrera"] == id_carrera:
            if not salon_ya_tiene_unidades(unidades, salon["id_salon"], id_plantilla):
                encontrados += 1
                print(
                    f"ID: {salon['id_salon']} | "
    ...
    if encontrados == 0:
        print("No hay salones disponibles. Todos ya tienen unidades para esta plantilla.")
...
def registrar_unidad():
    ...
    mostrar_carreras_desde_salones(salones)
    ...
    mostrar_plantillas_por_carrera(plantillas, id_carrera)
    ...
    mostrar_salones_disponibles(salones, unidades, id_carrera, id_plantilla)
