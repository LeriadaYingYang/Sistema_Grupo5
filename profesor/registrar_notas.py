...
def registrar_notas_profesor(profesor):
    ...
    id_unidad = pedir_id_entero("\nIngrese ID de la unidad: ")
    unidad = buscar_por_id(unidades, "id_unidad", id_unidad)
    
    if unidad is None or unidad["id_salon"] != id_salon:
        print("❌ Error: Unidad no encontrada en este salón.")
        pausa()
        return

    # 4. Seleccionar módulo
    modulos_unidad = [m for m in modulos if m["id_unidad"] == id_unidad and m["estado"] == "Activo"]
    ...
    alumnos_salon = [a for a in asignaciones if a["id_salon"] == id_salon and a["estado"] == "Activo"]
    ...
    if not en_salon:
        print("❌ Error: El alumno no está inscrito en este salón.")
        pausa()
        return
    ...
    while True:
        ...
        # Generar lista de opciones válidas para el menú (orden de las notas + 0 para salir)
        opciones_menu_validas = [0]

        for nota in registro["notas"]:
            valor = nota["nota"] if nota["nota"] != "" else "Sin nota"
            print(f"  {nota['orden']}. {nota['nombre_nota']:<25} -> {valor}")
            opciones_menu_validas.append(nota["orden"])
        ...
        orden = pedir_opcion_menu("\n  Seleccione número de nota a ingresar/modificar (0 para salir): ", opciones_menu_validas)
        ...
        # Como usamos pedir_opcion_menu, esto en teoría ya está cubierto, pero se mantiene por doble seguridad.
        if nota_obj is None:
